...
def eval_ood_detection(P, model, id_loader, train_loader=None, simclr_aug=None):
    ...
    feats_train, targets_train = get_features(P, f'{P.dataset}_train', model, train_loader, prefix=prefix, **kwargs)  # (M, T, d)
    ...
    feats_id, targets_id = get_features(P, P.dataset, model, id_loader, prefix=prefix, **kwargs)  # (N, T, d)
    scores_id = get_scores(P, feats_id).numpy()

    return scores_id, targets_id
...
def get_features(P, data_name, model, loader, interp=False, prefix='',
    ...
    if len(left) > 0:
        _feats_dict, targets = _get_features(P, model, loader, interp, P.dataset == 'imagenet',
...
def _get_features(P, model, loader, interp=False, imagenet=False, simclr_aug=None,
    ...
    for i, (x, targets) in enumerate(loader):
        ...
        targets_all += list(targets.detach().cpu().numpy())
